...
export const DocumentCard: React.FC<DocumentCardProps> = ({ document, onEdit }) => {
  ...
  const storageIsLink = !!document.storageLocation && isUrl(document.storageLocation);

  const handleCardClick = () => {
    if (storageIsLink) {
      window.open(document.storageLocation, '_blank', 'noopener,noreferrer');
    }
  };
  ...
  return (
    <>
      <div onClick={handleCardClick} className={`group bg-white dark:bg-gray-900 rounded-2xl border shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 ${config.bar} ${storageIsLink ? 'cursor-pointer' : ''}`}>
        {/* Urgency top bar */}
        ...
        <div className="p-5">
          ...
          <div className="flex items-start justify-between gap-2 mb-4">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={(e) => { e.stopPropagation(); onEdit(document); }} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
              </button>
              <button onClick={(e) => { e.stopPropagation(); setShowConfirm(true); }} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all">
          ...
          <div className="space-y-2 mb-4">
            ...
            {document.documentNumber && (
              <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); setDocNumberRevealed((v) => !v); }}
            ...
            {document.storageLocation && (
              <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                ...
                {storageIsLink && (
                  <svg className="w-3 h-3 shrink-0 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                )}
              </div>
            )}
          </div>

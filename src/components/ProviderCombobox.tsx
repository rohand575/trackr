  ...
  return (
    <div ref={containerRef} className="relative" onBlur={handleBlurContainer}>
      <input
        ...
        className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-lg bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 transition-colors"
      />
      {isOpen && (
...
const ProviderOption: React.FC<ProviderOptionProps> = ({ name, logoUrl, onSelect }) => {
  ...
  return (
    <button
      ...
      className="flex items-center gap-2.5 w-full px-3 py-2 text-sm text-gray-800 hover:bg-indigo-50 transition-colors text-left"
    >

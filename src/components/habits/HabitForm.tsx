    ...
    <div className="fixed inset-0 z-40 flex items-start justify-center px-4 py-6 overflow-y-auto">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-2xl animate-scale-in mt-4 mb-4">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">{editingHabit ? 'Edit Habit' : 'New Habit'}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Build consistency one day at a time</p>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4">
          {/* Name */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Habit Name <span className="text-red-500">*</span></label>
            ...
            <div className="flex flex-wrap gap-2">
              {PRESET_ICONS.map((icon) => (
                <button
                  ...
                  className={`w-9 h-9 rounded-lg text-lg flex items-center justify-center transition-all ${selectedIcon === icon ? 'bg-indigo-100 dark:bg-indigo-950/50 ring-2 ring-indigo-500 scale-110' : 'bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                >
                  {icon}
                </button>
              ))}
